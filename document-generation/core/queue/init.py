...
async def _ensure_stream(js: JetStreamContext, stream: dict) -> None:
    ...
    ``add_stream`` is NOT idempotent when a stream with the same name already
    exists with a *different* configuration (err 10058) — it raises. Because the
    ``nats_data`` volume persists streams across restarts, a config drift from an
    earlier iteration of this service would otherwise crash-loop the worker on
    boot (and it would never subscribe → nothing renders). We reconcile mutable
    changes (subjects, limits) via ``update_stream``; an immutable mismatch
    (retention/storage) surfaces a clear, actionable error. Mirrors billing's
    ``core/queue/init.py::_ensure_stream``.
    """
    try:
